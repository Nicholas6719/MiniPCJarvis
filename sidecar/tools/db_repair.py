...
def _copy_table(src: sqlite3.Connection, dst: sqlite3.Connection,
    ...
    marks = ",".join("?" * len(cols))
    insert = f'INSERT OR IGNORE INTO "{name}" ({collist}) VALUES ({marks})'

    def put(rows) -> tuple[int, int]:
...
def repair(db_path: str, *, keep_backup: bool = True,
    ...
    report: dict = {"tables": {}, "lost": 0, "kept": 0, "precious_lost": []}
    ...
    for name, _sql in schema:
        t0 = time.time()
        kept, lost = _copy_table(src, dst, name)
        log.info("%-16s kept %6d  lost %5d  (%.1fs)", name, kept, lost, time.time() - t0)
        report["tables"][name] = {"kept": kept, "lost": lost}
        report["kept"] += kept
        report["lost"] += lost
        if lost and name in PRECIOUS:
            report["precious_lost"].append(f"{name}:{lost}")
